...
Для корректной работы __StormGuardian Files__ рекомендуется версия PHP ```8.1``` и выше, можно и ниже, но не рекомендуется (некоторые функции могут быть недоступны). Минимальная версия PHP для корректной работы - ```7.4```.


#### ГОТОВО:
Здесь будут функции, которые уже готовы на стороне клиента(готово - :ballot_box_with_check:, не готово - :blue_square:) и/или на стороне бекенда(готово - :white_check_mark:, не готово - :green_square:).
...
### Изменения за 09.12.2023 - 10.12.2023:
- Теперь можно загружать файлы.
- Улучшена безопасность.
- Добавлен украинский язык.
- Улучшена страница для просмотра изображений.
- Функция ```get_files``` получила новый параметр - ```$mimetype```, что позволяет получать файлы только одного типа.

### Изменения за 08.12.2023:

...
Для корректной работы __StormGuardian Files__ рекомендуется версия PHP ```8.1``` и выше, можно и ниже, но не рекомендуется (некоторые функции могут быть недоступны).
